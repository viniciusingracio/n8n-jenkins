...
if [ -d /var/bigbluebutton/published/mconf_encrypted/ ]; then
    find /var/bigbluebutton/published/mconf_encrypted/ -maxdepth 1 -mtime +60 -exec rm -rf {} \;
fi

if [ -d /var/bigbluebutton/unpublished/mconf_encrypted/ ]; then
    find /var/bigbluebutton/unpublished/mconf_encrypted/ -maxdepth 1 -mtime +60 -exec rm -rf {} \;
fi
